...
class Gesture {
  ...
  constructor(threshold, autoSetMode) {
    ...
    // Mouse event support for systems that convert touch to mouse
    var self = this
    var createFakeTouch = (evt) => {
      return {
        identifier: 0,
        pageX: evt.pageX,
        pageY: evt.pageY,
    ...
    var createFakeTouchEvent = (evt) => {
      var fakeTouch = createFakeTouch(evt)
      return {
        target: evt.target,
        timeStamp: evt.timeStamp,
        touches: self._mouseDown ? [fakeTouch] : [],
        changedTouches: [fakeTouch],
        composedPath: () => evt.composedPath()
      }
    }
    this._dom.addEventListener("mousedown", (evt) => {
      if (evt.button !== 0) return
      self._mouseDown = true
      this._handlerTouch("touch", createFakeTouchEvent(evt))
    }, true)
    this._dom.addEventListener("mouseup", (evt) => {
      if (!self._mouseDown) return
      self._mouseDown = false
      this._handlerRelease(createFakeTouchEvent(evt))
    }, true)
    this._dom.addEventListener("mousemove", (evt) => {
      if (!self._mouseDown) return
      this._handlerTouch("move", createFakeTouchEvent(evt))
    }, true)
    this._dom.addEventListener("mouseleave", (evt) => {
      if (!self._mouseDown) return
      self._mouseDown = false
      this._handlerCancel(evt)
    }, true)
...
/** Gesture Handler part **/
  ...
  _getDirection (sp, ep) {
    var moveX = (ep.x - sp.x)
    var moveY = (ep.y - sp.y)
    // Bias towards horizontal - vertical must be 1.5x horizontal to count as vertical
    if (Math.abs(moveX) >= Math.abs(moveY) * 0.67) {
      if (moveX >= 0) {
