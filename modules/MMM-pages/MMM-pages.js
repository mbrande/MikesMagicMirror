Module.register('MMM-pages', {
  ...
  notificationReceived(notification, payload) {
    switch (notification) {
      case 'PAGE_CHANGED':
        ...
        break;
      case 'PAGE_INCREMENT':
        // Debounce rapid page changes (ignore if less than 500ms since last change)
        if (Date.now() - this.lastPageChangeTime < 500) {
          Log.log('[MMM-pages] Ignoring rapid PAGE_INCREMENT');
          break;
        }
  ...
  animatePageChange(targetPageName) {
    ...
    Log.log(`[MMM-pages] Switching to page ${this.curPage}, showing: ${modulesToShow.join(', ')}`);

    const modulesToHide = MM.getModules().exceptWithClass(modulesToShow);
    Log.log(`[MMM-pages] Hiding ${modulesToHide.length} modules`);
    modulesToHide.enumerate(module => {
      Log.log(`[MMM-pages] Hiding: ${module.name}`);
      ...
      MM.getModules()
        ...
        .enumerate((module) => {
          ...
          if (!this.permanentlyHiddenModules.has(module.identifier)) {
            Log.log(`[MMM-pages] Showing: ${module.name}`);
